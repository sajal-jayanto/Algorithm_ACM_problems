...
int main()
{
    int t, n, temp , a , b, ar[MAX];
    int *low , *high;
    sf(t);
    for(int i = 0 ; i < t ; ++i) sf(ar[i]);
    sf(n);
    while(n--)
    {
        sf(temp);
        low = lower_bound( ar , ar + t , temp );
        high = upper_bound( ar , ar + t , temp );
        --low;
        a = low - ar;
        b = high - ar;
        if(low == ar + t || low - ar < 0) printf("X %d\n", ar[b]);
        else if(high == ar + t) printf("%d X\n", ar[a]);
        else printf("%d %d\n", ar[a], ar[b]);
    }
    return 0;
}
