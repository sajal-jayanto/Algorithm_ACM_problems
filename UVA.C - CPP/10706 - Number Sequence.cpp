...
int main()
{
    num[1] = 1;
    dig[1] = 1;
    for(int i = 2 ; i <= MAX ; ++i) dig[i] = num[i] = num[i - 1] + (ll) floor(log10(i)) + 1;
    for(int i = 2 ; i <= MAX ; ++i) num[i] = num[i] + num[i - 1];
    int t;
    ll n, temp, car;
    dig[0] = 0;
    sf("%d", &t);
    ...
    {
        sf("%lld", &n);
        temp = lower_bound(num + 1 , num + MAX , n) - num ;
        temp = n - num[temp - 1];
        car = lower_bound(dig + 1 , dig + MAX , temp) - dig;
        sprintf(str , "%lld", car);
        n = temp - dig[car - 1];
       // pf("[%lld %lld %lld]\n", temp, dig[car - 1], car);
        pf("%c\n", str[n - 1]);
