...
struct node {
    ...
    int cnt;
    node(){}
    node(string k , int a , int b) {name = k ; index = a ; cnt = b;}
    bool operator <(const node &t) const
    {
        if(cnt == t.cnt) return index > t.index;
        return cnt < t.cnt;
    }
};

map < string , pair < int , int > > ve;
...
int main()
{
    ...
    {
       // if(cc) pf("\n");
        pf("Case #%d: Dilbert should drink beverages in this order:", ++cc);
        for(int i = 1 ; i <= n ; ++i)
        {
            cin >> temp;
            ve[temp].se = i;
        ...
        {
            cin >> temp >> temp1;
            mp[temp].push_back(temp1);
            ve[temp1].fs++;
        }
        for(it = ve.begin() ; it != ve.end() ; ++it) if(!it->se.fs) Q.push(node(it->fs , it->se.se , it->se.fs));
        while(!Q.empty())
        {
            node k = Q.top();
            Q.pop();
            pf(" %s", k.name.c_str());
            int len = mp[k.name].size();
            for(int i = 0 ; i < len ; ++i)
            {
                temp = mp[k.name][i];
                ve[temp].fs--;
                if(!ve[temp].fs) Q.push(node(temp , ve[temp].se ,ve[temp].fs));
            }
        }
        pf(".\n\n");
        ve.clear();
