...
void marge(int low , int mid , int high)
{
    ...
    while(a <= mid  && c <= high)
    {
        if(ar[a] <= ar[c]) temp[b++] = ar[a++];
        else { temp[b++] = ar[c++];  sum += (mid + 1) - a; }
    }
    while(a <= mid) temp[b++] = ar[a++];
    while(c <= high) temp[b++] = ar[c++];
    for(int i = low ; i < b ; ++i) ar[i] = temp[i];
}
