...
vector < char > par;
int n, len, cnt[MAX];
void permutation(int k , int l , int n);
void print();
...
void permutation(int k , int l , int n)
{
    ...
    for(int i = l ; i < len ; ++i)
    {
        if(!par.empty() && par[par.size() - 1] > str[i]) continue ;
        if(!cnt[i])
        {
            cnt[i] = true ; par.push_back(str[i]);
            permutation(k + 1 , i + 1,  n);
            cnt[i] = false; par.pop_back();
