    ...
    while( sf("%d", &n) != EOF)
    {
        ...
        for(int i = 0 ; i < n ; ++i)
        {
            ...
            else
            {
                ...
                if(!S.empty())  y = S.top();
                if(!PQ.empty()) z = PQ.top();
                if(x == b && !Q.empty()){ ++q; Q.pop();}
                if(y == b && !S.empty()){ ++s; S.pop();}
                if(z == b && !PQ.empty()){ ++pq; PQ.pop(); }
                ++cnt;
            }
        }
        if(q != cnt && s != cnt && pq != cnt) pf("impossible\n");
        else if((q == cnt && s == cnt) || (q == cnt && pq == cnt) || (s == cnt && pq == cnt)) pf("not sure\n");
        else if(q == cnt) pf("queue\n");
        else if(s == cnt) pf("stack\n");
        else pf("priority queue\n");
        //pf("%d %d %d [%d]\n", q , s , pq, cnt);
        while(!Q.empty()) Q.pop();
        while(!S.empty()) S.pop();
