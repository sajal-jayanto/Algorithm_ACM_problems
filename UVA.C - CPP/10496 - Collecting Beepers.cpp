...
int DP[12][4095];
int an;
int call(int p , int mask , int temp);
...
int main()
{
    ...
    {
        sf("%d %d" , &temp , &temp);
        sf("%d %d", &pos[0].x , &pos[0].y);
        sf("%d", &temp);
        for(int i = 1 ; i <= temp ; ++i) sf("%d %d" , &pos[i].x , &pos[i].y);
        mem(DP , -1);
        an = (1 << (temp + 1)) - 1;
...
int call(int p , int mask , int temp)
{
    if(mask == an) return dis(pos[p] , pos[0]);
    int &ret = DP[p][mask];
    if(ret != -1) return ret;
    ret = (1 << 28);
    for(int i = 1 ; i <= temp ; ++i)
    {
        if(!bit_check(mask , i)) ret = min(ret , dis(pos[p] , pos[i]) + call(i , bit_set(mask , i) , temp));
    }
    return ret;
}
