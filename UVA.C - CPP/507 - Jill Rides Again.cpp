...
int main()
{
    ...
    {
        sf("%d", &n);
        for(int i = 0 ; i < n - 1 ; ++i) sf("%d", &num[i]);
        Max_sum(n - 1);
        if(e <= se) pf("Route %d has no nice parts\n" , ++cc);
...
void Max_sum(int n)
{
    int maxs=0,sum=0; s=1,e=0,se=0;
    for(int i=0;i<n;++i)
    {
        sum=sum+num[i];
        if(sum<0) sum=0,s=i+2;
        else if(maxs<sum) maxs=sum,e=i+2,se=s;
        else if(maxs==sum&&(i+2-s)>(e-se))e=i+2,se=s;
    }
}
