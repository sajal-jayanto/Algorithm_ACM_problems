...
int DP[5][(1<<20)+2];
int n;
bool bit_musk(int musk , int leve , int sum , int get);
...
int main()
{
    ...
    while(t--)
    {
        ...
        sf("%d", &n);
        mem(DP , -1);
        for(int i = 0 ; i < n ; ++i) sf("%d", &num[i]) , sum = sum + num[i];
        if(sum % 4) pf("no\n");
        else
        {
            if(bit_musk(0 , 0 , 0 , sum / 4)) pf("yes\n");
            else pf("no\n");
        }
    }
    return 0;
...
bool bit_musk(int mark , int leve , int sum , int get)
{
    if(DP[leve][mark] != -1) return false;
    if(sum == get)
    {
        ++leve;
        sum = 0;
        if(leve == 4) return true;
    }
    for(int i = 0 ; i < n ; ++i)
    {
        if(!bit_check(mark , i) && sum + num[i] <= get)
        {
            if( bit_musk(bit_set(mark , i) , leve , sum + num[i] , get) ) return true;
        }
    }
    DP[leve][mark] = 0;
    return false;
