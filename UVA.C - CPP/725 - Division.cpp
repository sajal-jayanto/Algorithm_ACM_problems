...
int main()
{
    ...
    {
        int one = 0 ; int two = 0;
        for(int i = 0 ; i < 5 ; ++i) one = (one * 10) + num[i];
        for(int i = 5 ; i < 10 ; ++i) two = (two * 10) + num[i];
        if(one > two && one % two == 0)
        {
            temp = one / two;
            mymap[temp].push_back(node(one , two));
        }
    ...
    while(sf("%d", &n) && n)
    {
        if(cnt) pf("\n");
        temp = mymap[n].size();
        if(temp) for(int i = 0 ; i < temp ; ++i) pf("%05d / %05d = %d\n", mymap[n][i].x ,mymap[n][i].y, n);
        else pf("There are no solutions for %d.\n", n);
        ++cnt;
    }
    return 0;
}
