...
int main()
{
    ...
    #endif // org
    int h , m , s, speed ,  pr_h = 0, pr_m = 0, pr_s = 0, pr_speed = 0;
    double sum = 0.0;
    char str[MAX];
    while(gets(str))
    {
        ...
        if(len == 8)
        {
            ...
            sum = sum + ((h * 3600 + m * 60 + s) - (pr_h * 3600 + pr_m *60 + pr_s)) * (double) pr_speed / 3600.0;
            pr_h = h;  pr_m = m; pr_s = s;
            pf("%02d:%02d:%02d %.2lf km\n", h , m , s, sum);
        }
        else
        {
            sscanf(str , "%d:%d:%d %d" , &h , &m , &s , &speed);
            sum = sum + ((h * 3600 + m * 60 + s) - (pr_h * 3600 + pr_m *60 + pr_s)) * (double) pr_speed / 3600.0;
            pr_h = h;  pr_m = m; pr_s = s; pr_speed = speed;
        }
    }
    return 0;
