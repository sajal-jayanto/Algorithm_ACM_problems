...
int main()
{
    cnt.push_back(2); cnt.push_back(12);
    for(int i = 0 ; i < 2 ; ++i) cnt.push_back(3) , cnt.push_back(11);
    for(int i = 0 ; i < 3 ; ++i) cnt.push_back(4) , cnt.push_back(10);
    for(int i = 0 ; i < 4 ; ++i) cnt.push_back(5) , cnt.push_back(9);
    for(int i = 0 ; i < 5 ; ++i) cnt.push_back(6) , cnt.push_back(8);
    for(int i = 0 ; i < 6 ; ++i) cnt.push_back(7);
    sort(cnt.begin() , cnt.end());
    //for(int i = 0 ; i < cnt.size() ; ++i) pf("%d ", cnt[i]);
    int t, cn, ans ;
    char str[105];
    sf("%d", &t);
    getchar();
    while(t--)
    {
        ...
        mem(abc , 0);
        gets(str);
        for(int i = 0 ; str[i] ; ++i) if(str[i] != ' ') ++abc[str[i] - '0'];
        sort(abc , abc + 45);
        for(int i = 44 ; i >= 0  ; --i)
        {
            if(abc[i] != 0) ans = ans + (abc[i] * cnt[cn++]);
            else break;
        }
        pf("%d\n", ans);
    }
